...
global.b2link_router__admin__schema_create_html_selector_list_li._f0 = function( d, dbNm ){
	...
	var path = "../../../Development-NodeJS_Modules/js/b2link_router/_admin/_schema/ui_include/div-data_input_elements/";
	//var path = "../../WebPage/root/ui_include/div-data_input_elements/";
	...
	for( var colNm in db )
	{
		...
		if( col.hasOwnProperty( "properties" ) && col.properties )
		{
			...
			for( var pk in o0 )
			{
				...
				else if( "uint" == io.type )
				{
					s += '\n\t{{' + pk + '}}';
					if( 2 == io.values.length )
					{
					}
					else
					{
